...
export const analyzeLabel = action({
  ...
  handler: async (_ctx, args): Promise<ComplianceAnalysisResult> => {
    ...
    // ─── COMMODITY IDENTIFICATION ─────────────────────────────────────────
    const commodityType: CommodityType = (extraction.commodityCategory as CommodityType) || identifyCommodity(extraction);
    const commodityConfig = COMMODITY_CATEGORIES[commodityType] || COMMODITY_CATEGORIES.other;
    const { applicableRuleIds, exemptionsApplied, skippedRules } = selectApplicableRules(commodityType, extraction);

    // ─── MRP CURRENCY VALIDATION ──────────────────────────────────────────
    const mrpFieldValue = extraction.fields?.mrp?.value || "";
    const mrpCurrency = validateMRPCurrency(mrpFieldValue);

    // ─── COMMODITY-AWARE RULE EVALUATION ──────────────────────────────────
    // Only evaluate rules that are applicable to the identified commodity
    ...
    for (const ruleId of applicableRuleIds) {
      ...
      // Format/quality checks (rules 9, 10, 11, 12) are handled by validators
      if (ruleId === 9 || ruleId === 10 || ruleId === 11 || ruleId === 12) continue;
      if (fieldNames.length === 0) continue;

      // Evaluate the primary field for each rule
      ...
      const fieldStatus = evaluateField(primaryField, extracted as ExtractedFieldValue, ruleId, extraction.imageQuality || "unknown");

      // Check if this rule has an exemption for the current commodity
      const exemption = exemptionsApplied.find(e => e.ruleId === ruleId);
      if (exemption && fieldStatus.status === "non-compliant") {
        // Exemption applies — downgrade to review-required with exemption note
        fieldStatus.status = "review-required";
        fieldStatus.complianceReason = fieldStatus.complianceReason + " [EXCEPTION: " + exemption.reason + "]";
      }

      fieldResults.push(fieldStatus);
    }
    ...
    return {
      ...
      commodityInfo: {
        ...
        displayName: commodityConfig.displayName,
        description: commodityConfig.description,
        applicableRuleIds,
        exemptionsApplied,
        skippedRules,
        additionalFields: commodityConfig.additionalFields,
      },
