...
import type { ProductInfo, ComplianceResult, FieldDetectionStatus, ProductView } from "@/lib/types";
import {
  Upload,
  Camera,
  X,
  ...
  const runAIInspection = async () => {
    ...
    try {
      ...
      const aiResult = await analyzeLabel({
        images: [{ base64: base64, view: "front" }],
        productInfo: {
      ...
      // Map the result to our ComplianceResult type with all required fields
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {step === 3 && result && !isProcessing && (
        <>
          ...
          <div className={`glass-card rounded-2xl p-6 border ${result.status === "compliant" ? "border-green-200" : result.status === "non-compliant" ? "border-red-200" : "border-amber-200"}`}>
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="sm:ml-auto flex gap-2">
                <Button variant="outline" onClick={() => setShowReport(true)} className="rounded-xl text-xs"><FileText className="mr-1 h-3 w-3" /> View Report</Button>
                <Button onClick={resetInspection} variant="outline" className="rounded-xl text-xs"><RotateCcw className="mr-1 h-3 w-3" /> New Inspection</Button>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="glass-card rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {result.fields.map((field) => (
                  <div key={field.id} className={`p-3 rounded-xl border transition-all ${field.status === "compliant" ? "bg-green-50/40 border-green-200/50" : field.status === "review-required" ? "bg-amber-50/40 border-amber-200/50" : "bg-red-50/40 border-red-200/50"}`}>
                    ...
                    {editingId === field.id ? (
                      <div className="flex items-center gap-2">
                        <Input value={editingFields[field.id] ?? field.value} onChange={(e) => handleFieldEdit(field.id, e.target.value)} className="h-7 text-xs glass-input" />
                        <button onClick={() => saveFieldEdit(field.id)} className="p-1 rounded bg-green-100 text-green-600 hover:bg-green-200"><Save className="h-3 w-3" /></button>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between">
              ...
              <div className="mt-4 flex gap-2">
                <Button variant="outline" size="sm" onClick={runAIInspection} className="rounded-lg text-xs flex-1"><RotateCcw className="mr-1 h-3 w-3" /> Re-run AI Analysis</Button>
                <Button size="sm" className="rounded-lg text-xs flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white"><CheckCircle2 className="mr-1 h-3 w-3" /> Confirm Data</Button>
              </div>
            </div>
          </div>
                      ...
                      <div className="px-4 pb-4 space-y-3 border-t border-gray-100 pt-3">
                        ...
                        <div className="p-3 rounded-lg bg-blue-50/50 border border-blue-100">
                          <div className="flex items-start gap-2">
                            ...
                            <div><p className="text-[10px] font-bold text-blue-700 uppercase">Why was this flagged?</p><p className="text-xs text-blue-600 mt-1">{v.explanation}</p></div>
                          </div>
                        </div>
                        <div className="p-3 rounded-lg bg-amber-50/50 border border-amber-100"><p className="text-[10px] font-bold text-amber-700 uppercase">Recommendation</p><p className="text-xs text-amber-700 mt-1">{v.recommendation}</p></div>
                      </div>
                    )}
                  </div>
                ))}
      ...
      {showReport && result && (
        <ReportModal result={result} productInfo={productInfo} inspectionId={inspectionId.current} onClose={() => setShowReport(false)} />
      )}
    </div>
  );
}
...
function ReportModal({ result, productInfo, inspectionId, onClose }: {
  ...
  const handleDownloadPDF = () => {
    ...
    const violationsHtml = result.violations.length > 0 ? `<h3 style="font-size:14px;font-weight:700;color:#111827;margin:20px 0 10px">Violations (${result.violations.length})</h3>${result.violations.map((v) => `<div style="padding:10px;border-radius:8px;background:#fef2f2;border:1px solid #fecaca;margin-bottom:8px"><p style="font-size:12px;font-weight:700;color:#991b1b">${v.title}</p><p style="font-size:11px;color:#b91c1c;margin-top:4px">${v.explanation}</p><p style="font-size:10px;color:#9ca3af;margin-top:4px">Recommendation: ${v.recommendation}</p></div>`).join("")}` : `<p style="color:#16a34a;font-weight:700">No violations detected.</p>`;
    printWindow.document.write(`<!DOCTYPE html><html><head><title>Inspection Report - ${inspectionId}</title><style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:40px;color:#111827}@media print{body{margin:20px}}</style></head><body><div style="text-align:center;margin-bottom:20px"><h1 style="font-size:20px;font-weight:800;margin:0">MetrologyAI</h1><p style="font-size:11px;color:#6b7280;margin:4px 0 0">AI-Assisted Legal Metrology Inspection Report</p></div><div style="border-top:2px solid #e5e7eb;border-bottom:1px solid #e5e7eb;padding:12px 0;margin-bottom:16px;display:flex;justify-content:space-between"><span style="font-size:12px;color:#6b7280">Inspection ID: <strong>${inspectionId}</strong></span><span style="font-size:12px;color:#6b7280">Date: <strong>${productInfo.dateTime || new Date().toLocaleDateString()}</strong></span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px"><div style="padding:10px;border-radius:8px;background:#f9fafb"><p style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;margin:0">Product</p><p style="font-size:13px;font-weight:600;margin:4px 0 0">${productInfo.productName || "N/A"}</p><p style="font-size:11px;color:#6b7280;margin:2px 0 0">${productInfo.manufacturer || ""}</p></div><div style="padding:10px;border-radius:8px;background:#f9fafb"><p style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;margin:0">Inspector</p><p style="font-size:13px;font-weight:600;margin:4px 0 0">${productInfo.inspectorId || "N/A"}</p><p style="font-size:11px;color:#6b7280;margin:2px 0 0">${productInfo.location || ""}</p></div></div><div style="padding:12px;border-radius:8px;border:2px solid ${statusColor}20;background:${statusColor}08;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><div><p style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;margin:0">AI Compliance Risk Score</p><p style="font-size:24px;font-weight:800;margin:4px 0 0">${result.score} / 100</p></div><span style="padding:4px 12px;border-radius:9999px;font-size:11px;font-weight:700;background:${statusColor}18;color:${statusColor}">${statusLabel}</span></div><h3 style="font-size:14px;font-weight:700;color:#111827;margin:20px 0 10px">Extracted Declarations</h3><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="border-bottom:2px solid #e5e7eb"><th style="text-align:left;padding:8px;font-weight:700;color:#6b7280;font-size:10px;text-transform:uppercase">Field</th><th style="text-align:left;padding:8px;font-weight:700;color:#6b7280;font-size:10px;text-transform:uppercase">Value</th><th style="text-align:center;padding:8px;font-weight:700;color:#6b7280;font-size:10px;text-transform:uppercase">Confidence</th><th style="text-align:center;padding:8px;font-weight:700;color:#6b7280;font-size:10px;text-transform:uppercase">Status</th></tr></thead><tbody>${fieldsHtml}</tbody></table>${violationsHtml}<div style="margin-top:24px;padding:12px;border-radius:8px;background:#eff6ff;border:1px solid #bfdbfe"><p style="font-size:10px;color:#1d4ed8;line-height:1.6;margin:0"><strong>AI Assistance Notice:</strong> This report is generated using AI-assisted extraction and rule-based analysis. Final verification and enforcement decisions remain with the authorized inspector.</p></div><div style="margin-top:16px;text-align:center;font-size:9px;color:#9ca3af"><p style="margin:0">MetrologyAI — AI-Assisted Legal Metrology Inspection System</p><p style="margin:2px 0 0">Generated on ${new Date().toLocaleString()}</p></div></body></html>`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto glass-card rounded-3xl p-6 sm:p-8 space-y-6">
              ...
              <tbody>
                {result.fields.map((f) => (
                  <tr key={f.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${f.status === "compliant" ? "bg-green-100 text-green-700" : f.status === "review-required" ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
        ...
        {result.violations.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-gray-900 mb-3">Violations ({result.violations.length})</h3>
            <div className="space-y-2">
              {result.violations.map((v) => (
                <div key={v.id} className="p-3 rounded-xl bg-red-50/50 border border-red-100">
                  ...
                  <p className="text-[10px] text-red-600 mt-1 ml-5">{v.explanation}</p>
                  <p className="text-[10px] text-gray-500 mt-1 ml-5">Recommendation: {v.recommendation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-3 rounded-xl bg-blue-50/50 border border-blue-100">
          <p className="text-[10px] text-blue-700" style={{ lineHeight: "1.6" }}>
        ...
        <div className="flex justify-between pt-2">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={handleDownloadPDF} className="rounded-xl text-xs"><Icons.Download className="mr-1 h-3 w-3" /> Download PDF</Button>
            <Button onClick={handleDownloadPDF} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold px-4 rounded-xl text-xs"><Icons.FileText className="mr-1 h-3 w-3" /> Print Report</Button>
          </div>
        </div>
      </div>
    </div>
